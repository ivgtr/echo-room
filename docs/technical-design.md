...
| 音声 | howler.js | 音声再生、音量系統、フェード、ループ、形式切替 |
...
type GameEvent =
  ...
  | { type: 'AUDIO_REPLAY_REQUESTED'; audioId: AudioId }
...
- PixiJSのアクセシビリティ用DOMオーバーレイを有効にし、各対象へ名前、役割、フォーカス順を与える。
...
- ID参照の存在、重複、到達不能、音声と字幕の対応、20分差を追加検証する。
...
lines:
  - speaker: future_protagonist
    text: ……聞こえるか？
    audio: voice/packet_01.m4a
    effects: [intercom_noise]
  - speaker: protagonist
    text: 誰だ？
    audio: voice/protagonist_who.m4a
effectsOnComplete:
...
- 音声が未完成でも字幕だけで試作できる。
- `audio`が指定された行は、音声ファイルの存在と字幕IDの対応を検証する。
- 台詞の表示時間は音声長を優先し、字幕のみの場合は文字数から初期値を算出して個別調整できる。
...
| 音声パケット | media inspection | React | PACKET 04確認後に会話開始 |
...
- 各系統は0〜100の設定値を持ち、最終音量はmasterとの積で求める。
- 重要台詞中は環境音を一時的に下げる。
- ページ非表示または一時停止で全音声を停止し、復帰時に意図せず台詞の途中から再開しない。
- 環境音はシームレスループし、視点移動では原則として継続する。

### 10.2 ブラウザ制約への対応

- 初回の「ゲーム開始」操作を音声利用開始のユーザージェスチャーとして使う。
- 音声が開始できない場合は、再試行ボタンを表示し、字幕だけでも進行可能にする。
- howler.jsのWeb Audio利用とHTML5 Audioフォールバックを利用する。
- 音声の読込完了をゲーム進行の必須条件にせず、失敗時も字幕へフォールバックする。

### 10.3 素材形式

- 制作原本は非圧縮WAVで保持する。
- 配信用はOpus系とAAC系の2形式を用意し、ブラウザが再生可能な形式を選ぶ。
- 長い環境音と台詞は個別ファイル、短い共通効果音は必要に応じてAudio Sprite化する。
- 全音声に安定したIDを付け、字幕データと紐づける。

### 10.4 声紋演出

- 主人公と未来の主人公は同じ音声素材処理系を使う。
- 未来音声の序盤加工は、EQ、帯域制限、ノイズ、軽い歪みの組み合わせとする。
- 正体判明時は元音声へクロスフェードし、別音源の急な切替に聞こえないようにする。
- フィルターを外しても音量が急変しないよう事前にラウドネスを揃える。
...
- `title`: タイトル背景、開始音
- `room_emergency`: 暗い室内、非常灯、導入音声
- `room_powered`: 点灯後の室内、端末、ロッカー
- `reveal`: 音声解析と最終装置
- `ending`: 白飛び、タイトル、終幕音声
...
- 高密度画面でも描画解像度へ上限を設ける。
- 点滅、ノイズ、視差の品質設定を用意する。
- リリース前に実素材を使用してGPUメモリと音声メモリを測定する。
...
- 探索対象にはPixiJSのアクセシビリティオーバーレイを使用する。
...
| T-04 | 音声開始操作とSafariの再生制約 | iOS Safari実機で開始・復帰を確認 | Phase 2 |
| T-05 | バッテリー00:00後の演出 | 初見テストで緊迫感が失われないか確認 | Phase 2 |
| T-06 | YAML編集の使いやすさ | 台詞修正とパズル追加を非実装担当者にも試してもらう | Phase 3 |
| T-07 | 全音声を含む配信容量 | 本番相当の台詞数で形式と音質を比較 | Phase 4 |
| T-08 | 声の加工解除が正体の発見として機能するか | 本番演者の仮収録で聴取テスト | Phase 4 |
