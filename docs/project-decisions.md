...
| サウンド | ゲーム開始・再開操作で解放する単一Web Audio Sound Managerから、手続き生成のenvironment/effectsだけを再生する。文字送り音は話者差のない共通音とし、人物の声を表現しない。再生失敗時も字幕と視覚補助で進行できる | technical-design 10章 |
...
| シナリオ | 主要10問、20分差、未来の主人公、4 PACKET、単一endingを変更しない | requirements 2章・7章・8章 |
...
| D-02 | 発話音声は使用せず、台詞はHTML字幕で提示する。音響は環境音・効果音・通信ノイズだけとし、声紋はPACKET内特徴量と職員記録の視覚照合で成立させる | なし |
| D-03 / G-D01 | `protagonist_unknown`を正規IDとして維持する。32歳前後の日本人男性、短い黒髪、地下研究施設の設備保守・運用担当、濃灰の作業着とし、同一写真原本を職員証と声紋解析に使う | 氏名と事故前の詳細な経歴は定めない |
...
| D-08 | 全主要checkpoint直後の進行自動保存1枠と、即時反映する設定保存1枠を分離する。10問進行schema v3と設定schema v4の現行形式だけを受理し、進行消去後も設定・既読状態を保持する | なし |
...
現在、P0〜P4を止める未解決事項はない。真相、ending、主要パズル、技術基盤、対応環境を変更する必要が生じた場合は、実装計画4章に従って作業を停止し、判断を求める。
