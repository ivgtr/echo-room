...
- ボタン、タブ、フォーカス枠、波形の数列同等表示など、画面サイズや操作状態に応じて変化する単純形状はHTML/CSSで描画する。
...
- 開始時は北壁のドアを正面に見る。
- 北側中央にドア、その上に非常ロック・バッテリー表示。
- 北壁右寄りにインターホン。
- 東側に端末、その脇に開閉可能なパネル。
- 南側に机、北西寄りの見える位置に時計。
- 西側にロッカーとブレーカー。
- 窓はない。
- E-01の左右は機械設備のある厚い壁で、隣室への扉、窓、通路を描かない。
...
- 確認用スクリーンショット、比較sheet、生成途中の候補、デバッグ用書き出しなど、正規成果物ではない一時ファイルはプロジェクトルートの`tmp/`へ集約する。
- プロジェクトルート直下やOS側の`/tmp`へ一時制作物を散在させない。ツール実行前に必要ならプロジェクトルートで`mkdir -p tmp`を行う。
- `tmp/`の内容は`.gitignore`でGit管理外とし、本番build、asset validation、素材manifestから参照しない。
- 承認されて恒久成果物となるファイルは、用途に応じて`artwork/`、`public/assets/`、`docs/`などの正規配置先へ移し、命名・検収・commitの対象とする。
- 作業完了時に一時ファイルを残す場合は、依頼者が確認できるようプロジェクトルート基準のパスを伝える。
...
| GFX-CLOSE-008 | 端末 | 電源OFF、起動中、ON。画面内容はDOMで重ねる |
| GFX-CLOSE-009 | 端末横パネル | 閉、ドライバーで開いた状態 |
| GFX-CLOSE-010 | 声紋校正パネル | 閉、校正中、照合済み |
...
| GFX-ITEM-002 | 職員証の表面 | カード外装を生成し、文字と写真は後合成 |
...
| GFX-UI-005 | 搬送波・PACKET指紋・声紋特徴量 | HTML/CSSまたはCanvasで制作し、数列を併記 |
...
- 主人公の職員証写真に必要な外見設定（D-03 / G-D01で確定済み）
- 研究所ロゴを作るか、文字だけにするか
- 画面上の最終フォント
