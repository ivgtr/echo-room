...
| 本番音声 | 字幕と無音・仮音で進行 | 声紋の聴感評価と最終音声調整のみ |
...
- Zod、howler.js、Vitest、Playwright。
...
- 本番音声がない台詞は、字幕と再生完了を模した時間情報で進行できる。
...
- 受信・送信ログの20分差。
- 壁時計02:17。
- SYSTEMの02:17、02:37、-00:20:00。
- ロッカー入力0237。
- ドライバー、職員用カード、フロア図の取得。
...
- VOICE ANALYSISスイッチ。
- 98%、99%、100%の解析演出。
- E-01 OCCUPANTと仮職員証写真。
- 正体判明会話。

#### P3-06 Final PuzzleとEnding
...
- voice、environment、effectsの登録。
- 冒頭と終盤で同じ台詞・同じ演者であることを確認。
- 通信filterと正体判明時の解除。
- 字幕、再生時間、話者、audio IDを照合。
- 読込失敗時の字幕fallback。
...
- タブレット横。
- スマートフォン横。
- iOS Safariで音声開始、background復帰、保存を実機確認。
...
2. 時刻ログからロッカー解錠。
3. フロア図から隣室不存在の発見。
4. PACKET 04から未来通信の確信。
5. 声紋解析から正体判明。
6. 4 packet送信からending。
...
- UI componentからlocalStorage、howler、PixiJSを直接操作しない。
...
- 必須の代替素材、仮文言、仮音声が残っていない。
- SVGを本番グラフィック素材として使用していない。
- 画像と音声のID、寸法、字幕がmanifestと一致する。
...
- [ ] PixiJS、XState、Zod、howler.jsを導入する。
