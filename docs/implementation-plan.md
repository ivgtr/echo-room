...
- 二層図で通信線をECHO BUFFER RETURNまで追跡する。
...
- 設備状態を根拠に因果会話を再構成する。
- 4受信窓、-00:20:00、ECHO BUFFER RETURNを組み合わせる。
...
- 机の資料を進行に応じて電源表、同期調整メモ、夜間点検順へ切り替える。
- ロッカーの四記号を端末、通話器、BUFFER回路、ドアの銘板から取得できるようにする。
...
- ロッカーの解答情報は既存のデスク資料と各設備の個別調査へ保ち、背景記号、探索画面の常時銘板、装置内対応表、入力数を言い換える`4 STEP`を追加しない。
...
- 送信文編集レールを削除し、会話順、受信窓、遅延、終端を最終送信装置へ統合する。
- 旧Puzzle ID、Stage、hint、UI、test、保存分岐を削除し、進行schema v4へ破壊的に更新する。

#### P3B-02 テンポ・誤答・停滞UX

- 謎解き後の設備通知、独白、発見、重要会話は単一NarrativePanelへ統合し、プレイヤーの「続ける」で進める。
...
- 非常電源のケーブル隔離と投入順入力を削除し、初期状態でONのDOOR回路を切って残る3回路を任意順でONにする単一の四連ブレーカー盤へ破壊的移行する。
- 盤面にある回路別の物理ランプ数で概算負荷を示し、文書に記載した7 UNIT容量と照合させる。DOOR短絡中は他レバーを動かさず局所エラーと既存Sound Managerの失敗cueを返す。
- 成功後の端末、インターホン、BUFFER起動順は入力規則ではなく装置の復旧アニメーションとして示す。
- `gfx-close-005`と`gfx-close-007`を既存接写から参照編集し、見えるレバー、灯火、押しボタン、ハンドルはカメラ固定のラスター状態差分を組み合わせる。HTML/CSSは透明な入力面、記号、短い状態遷移だけに限定する。
...
1. 新規開始から電源復旧。
2. 搬送波同期と保守記号からロッカー解錠。
3. ログ指紋照合と二層図から20分差・回線帰還を発見。
4. PACKET復元と時系列照合から未来通信を確信。
5. 声紋特徴量の校正から正体判明。
6. 因果会話と送信窓設計からending。
