...
- P0〜P3は完了し、代替画像・字幕・仮サウンドで冒頭からエンディングまで通して遊べる。
...
- P5は全作業完了。次の着手点はP4-02の正式高解像度原本・layer分離とP4-04の環境音・効果音統合。
...
| P4-04 | in_progress | 発話音声は使用せず、voice系統・会話の`audioId`・howler.jsを撤去。設定schema v4をSOUND master、effects、environmentだけに単一化。SIGNALで字幕と信号、VOICE ANALYSISでPACKET内声紋特徴量と職員記録の照合を示す構造へ更新 | Node 24で`npm run check`成功（14 files/41 tests、4 bundle/27画像）。Chromium E2E 12件でSIGNAL・声紋照合・設定v4保存・無サウンド・全編進行を検証。SOUND設定を1280×720で目視確認 | 本番の環境音・効果音素材と統合 |
...
- 本番画像・サウンド: 正規IDと寸法のCanvas製代替画像、字幕、Web Audio仮音で実装を継続する。
...
2. P4-02の正式高解像度原本・layer分離・hit mask・残状態差分と、P4-04の環境音・効果音統合を完了してP4 Gateを閉じる。発話音声は追加しない。
3. P5は完了。進行はv2、設定はv4だけを正規形式とし、旧fixtureや旧読込分岐を追加しない。
4. `tmp/p5-02-motion-settings.png`、`tmp/p5-03-intro-skip.png`、`tmp/p4-04-sound-settings.png`は実画面確認用。その他の一時画像も引き続きignore済みの`tmp/`へ集約する。
5. P4-04の環境音・効果音は未統合で、P4全体が完了するまでP6 Gateは閉じたままとする。
