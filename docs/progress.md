...
| P4-04 | completed | 発話音声・voice系統・会話`audioId`・howler.js・旧tone playerを削除。設定schema v4のSOUND master、effects、environmentに対応する単一Sound Managerを実装し、非常電源・復旧後の機械ハムと通信・接続・回路・電源・ロック・解析・送信・ドア解錠cueを手続き生成。SYSTEM・非表示・title・OFFで再生中nodeを停止し、再開操作で復帰。SIGNAL字幕とPACKET声紋特徴量照合を維持 | Node 24で`npm run check`成功（15 files/45 tests、4 bundle/27画像、build 578.97kB）。Sound Manager unit 4件で解放・2 bus音量・pause/OFF停止・復帰・電源差分・非発話cue限定を検証。Chromium E2E 13件成功（全編keyboard、touch、SYSTEM、visibility、設定保存、無音進行、実ブラウザSound lifecycle） | なし |
