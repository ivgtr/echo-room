...
| P3B-02 | completed | ambient・subtitle・dramatic表示、誤答入力保持と局所反応、60秒無操作・90秒滞在の診断通知を実装。ロッカー完了時は右下のゲーム内メッセージ表示中も探索入力を維持し、取得画面が実表示された時だけ入力を止める | Narrative分類unit、誤答保持unit・ロッカー完了直後の移動を含む全編E2E、診断timer unit | なし |
