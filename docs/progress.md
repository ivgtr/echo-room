...
- UIQ-08は完了。進行に応じて一枚の紙を差し替える旧机UIと紙取得後素材を削除し、三つの手掛かり、二つの日常メモ、顔を見せない作業写真を最初から固定配置した。資料カード、紙面上の`読む`、矩形選択枠も撤去し、六点を固有の紙面、自然な重なり、紙自体の浮きと影、同じ物を中央へ持ち上げる再読操作へ変更した。紙面は固定し、装置を知った後は主人公の解釈だけを変える。SYSTEMには手掛かり三点だけを保存し、広角にも同じ紙束と写真の固定layerを置く。
...
| UIQ-08 | completed | 机を進行非依存の固定探索地点へ再設計。引き継ぎ、波形、戸締まりの三資料に、交代連絡、買い物メモ、作業写真を混在させた。全六点を施設用紙、方眼紙、チェック表、付箋、破れた私物メモ、白縁写真へ描き分け、資料カード、`読む`、矩形選択枠、別資料画面への置換を削除。同じ物を中央へ持ち上げる閲覧、机内のfocus復帰、進行別の主人公解釈、SYSTEMへの三資料保存、広角の固定紙束を統合した | `npm run check`成功（19 files/72 tests、4 bundle/31画像、production build）。Chromiumの机・時計E2Eと一時visual test 2件が成功。1280×720、844×390で机全景、引き継ぎ、写真を目視確認 | なし |
