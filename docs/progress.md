...
| P3B-04 | completed | 非常電源をDOOR短絡解除＋3補助回路の任意順投入へ再設計。CSS製機械部品を撤去し、カメラ固定の生成ラスター状態差分と透明hit areaへ統合。ロッカーも生成錠盤上の記号・入力面だけに単純化 | `npm run check`、PuzzleDevice unit、Chromium電源誤答・復旧・再開E2E、1280×720の初期・拒否状態を目視確認 | 初見時に物理ランプ数を負荷として認識できるかP7-01で確認 |
