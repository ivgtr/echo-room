...
- UIQ-07は完了。調査開始時の途切れた矩形枠を削除し、対象別の単純な演出用輪郭を約150msで一周するシアン線、小さなひし形、少し重ねて始まるカメラ接近へ置き換えた。操作判定用polygonは可視化せず、動き軽減時は静止表示とクロスフェードを使用する。
...
| UIQ-07 | completed | 調査開始時の途切れた矩形枠を、機器類の四角形、時計の円、机の台形を約150msで一周するCanvas輪郭、中央のひし形、120ms後から重なるカメラ接近へ置換。操作判定polygonと演出輪郭を分離し、動き軽減時は静止輪郭とひし形を表示 | Node 24で`npm run check`成功（18 files/70 tests、content・asset validation、production build）。Chromium E2E 3件（通常探索、連打防止・focus復帰、resize・reduced-motion）成功。1280×720のドア調査を目視確認 | なし |
