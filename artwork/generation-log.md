...
- 出力: `rejected/gfx-ref-001__candidate-c-lowpoly-preview.png`（ローカル保管、Git管理外）
...
- 出力: `rejected/gfx-ref-001__candidate-c-lowpoly-v2-preview.png`（ローカル保管、Git管理外）
...
- 出力: `rejected/gfx-ref-001__candidate-c-lowpoly-v3-preview.png`（ローカル保管、Git管理外）
...
- 出力: `rejected/gfx-ref-001__candidate-c-lowpoly-v4-preview.png`（ローカル保管、Git管理外）
...
- 初稿: `rejected/gfx-ref-001__candidate-c-cinematic-v5-preview.png`（ローカル保管、Git管理外）
...
previewはすべて1672×941。Git管理には初期3候補、承認版v6、contact sheetを残し、途中試行は`artwork/rejected/`へローカル保管する。シネマティック表面調整v6の承認後、これを単一参照としてGFX-REF-001 masterとGFX-REF-002を制作し、仕様の3840×2160以上を満たす。単純な拡大補間は行わない。
...
- runtime: 17状態を品質88のWebPとして`public/assets/images/close/`へ書き出し、`room_closeups` bundleへ登録。画素が同一のCLOSE-009 openとCLOSE-010 offは1ファイルを2 IDで共有する。breaker、locker lock、terminal、analysis、final transmissionの既存HTML UI背景へ接続した。
